...
import { Save, Image as ImageIcon, Clock, Headphones, Play, FileText, Loader2, Sparkles, RefreshCw } from "lucide-react";
...
function AddNewSong() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            ...
            if (response.success) {
                toast.success(response.message || "Tạo bài hát thành công");
                form.reset();
            } else {
                toast.error(response.message || "Tạo bài hát thất bại");
            }
        } catch (error: any) {
                            ...
                            render={({ field }) => (
                                <FormItem className="w-full">
                                    ...
                                    >
                                        ...
                                        <SelectContent
                                            className="bg-grayDarker border-grayDark/20 text-white placeholder:text-grayDark/50 focus-visible:ring-primary focus-visible:border-primary/50 transition-all max-h-60"
                                        >
                                            {authors.length === 0 ? (
